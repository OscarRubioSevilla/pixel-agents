...
export class HookEventHandler {
  ...
  private getProvider(providerId: string): HookProvider {
    return this.providers.get(providerId) ?? this.defaultProvider;
  }

  private getAgentProvider(agent: AgentState): HookProvider {
    return this.providers.get(agent.providerId ?? this.defaultProvider.id) ?? this.defaultProvider;
  }
  ...
  handleEvent(providerId: string, event: HookEvent): void {
    ...
    if (normEvent.kind === 'sessionStart') {
      ...
      // Cursor sessions: adopt immediately on sessionStart (no confirmation wait).
      if (provider.id === 'cursor' && (transcriptPath || cwd) && tracked) {
        if (debug)
          console.log(
            `[Pixel Agents] Hook: SessionStart -> adopting cursor session ${sid}... immediately`,
          );
        ...
        const adopted = [...this.agents.values()].find((a) => a.sessionId === event.session_id);
        if (adopted) {
          adopted.hookDelivered = true;
          this.registerAgent(event.session_id, adopted.id);
        }
        return;
      }
    ...
    switch (normEvent.kind) {
      ...
      case 'subagentStart':
        return provider.team
          ...
          : this.handleBasicSubagentStart(normEvent, agent, agentId);
      case 'subagentEnd':
        return provider.team
          ? this.handleSubagentStop(agent, agentId)
          : this.handleBasicSubagentStop(normEvent, agent, agentId);
  ...
  private handlePreToolUse(
    ...
    const skipTaskForJsonl = toolName === 'Task' || toolName === 'Agent';
    const sendTaskStart = agent.hooksOnly && skipTaskForJsonl;
    if (!skipTaskForJsonl || sendTaskStart) {
  ...
  private handlePostToolUse(
    ...
    const toolId = normEvent.toolId !== 'current' ? normEvent.toolId : agent.currentHookToolId;
    if (toolId) {
      ...
      if (agent.currentHookToolId === toolId) {
  ...
  /**
   * Within-turn subagents for providers without Agent Teams (e.g. Cursor hooks-only).
   */
  private handleBasicSubagentStart(
    normEvent: Extract<AgentEvent, { kind: 'subagentStart' }>,
    agent: AgentState,
    agentId: number,
  ): void {
    const provider = this.getAgentProvider(agent);
    let parentToolId: string | undefined = normEvent.parentToolId;
    if (parentToolId === 'current') {
      parentToolId = agent.currentHookToolId;
      if (!parentToolId) {
    ...
    const status = provider.formatToolStatus(normEvent.toolName, normEvent.input);
    ...
    subNames.set(subToolId, normEvent.toolName);
  ...
  private handleBasicSubagentStop(
    normEvent: Extract<AgentEvent, { kind: 'subagentEnd' }>,
    agent: AgentState,
    agentId: number,
  ): void {
    const provider = this.getAgentProvider(agent);
    let parentToolId = normEvent.parentToolId;
    if (parentToolId === 'current') {
      const parentTools = this.getSubagentToolSet(provider);
      for (const [toolId, toolName] of agent.activeToolNames) {
        if (parentTools.has(toolName) && agent.activeSubagentToolIds.has(toolId)) {
          parentToolId = toolId;
          break;
        }
      }
  ...
  private markAgentWaiting(agent: AgentState, agentId: number): void {
    ...
    const parentTools = this.getSubagentToolSet(this.getAgentProvider(agent));
